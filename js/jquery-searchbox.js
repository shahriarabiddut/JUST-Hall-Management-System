(function($) {
		...
		*/
		...
		var init = function (obj) {
			...
			// 絞り込み検索用のテキスト入力欄の追加
			self.before('<input type="text" class="refineText formTextbox form-control" style="width:100%!important;" />');
			...
			var visibleTarget =self.find('option').map(function(i, e) {
				return '<div class="bg-dark text-white " data-selected="off" data-searchval="' + $(e).val() + '"><span>' + $(e).text() + '</span></div>';
			}).get();
			self.after($('<div class="searchBoxElement py-1"></div>').hide());
			...
			parent.find('.searchBoxElement').css('width', '100%');
			...
			var changeSearchBoxElement = function() {
				...
				var selectedOption = self.find('option:selected');
				if(selectedOption){
					parent.find('.searchBoxElement').find('div').removeClass('selected');
					parent.find('.searchBoxElement').find('div[data-searchval="' + selectedOption.val() + '"]').addClass('selected');
				}
				
				// ダミーリスト選択時
				parent.find('.searchBoxElement').find('div').click(function(e){
					e.preventDefault();
					// e.stopPropagation();
					var div = $(this),
						searchval = div.data('searchval');
					self.val(searchval).change();
					parent.find('div').attr('data-selected', 'off');
					div.attr('data-selected', 'on');
				});
